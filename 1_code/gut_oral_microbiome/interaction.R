...
plot_combined_feature_selection_results <- function(results) {
 ...
 # 筛选显著协同效应的代谢物
 significant_metabolites <- subset(gut_oral_results_summary_co_influence, 
                                   R2_diff > quantile(R2_diff, 0.75))  # 使用四分位数或其他阈值
 
 
 
 analyze_interactions <- function(model, gut_features, oral_features) {
   interactions <- gbm.interactions(model)
   
   # 筛选出肠道-口腔菌群间的交互
   gut_oral_interactions <- interactions[
     grepl("gut_", interactions$var1) & grepl("oral_", interactions$var2) |
       grepl("oral_", interactions$var1) & grepl("gut_", interactions$var2),
   ]
   
   return(gut_oral_interactions)
 }
 
 
 for (metabolite in significant_metabolites$metabolite) {
   model <- combined_results$detailed_results[[metabolite]]$model
   
   # 提取特征重要性
   importance <- summary(model)
   gut_features <- importance$var[grepl("gut_", importance$var)]
   oral_features <- importance$var[grepl("oral_", importance$var)]
   
   # 分析交互
   interactions <- analyze_interactions(model, gut_features, oral_features)
   
   # 排序找出最重要的交互对
   top_interactions <- interactions[order(-interactions$interaction_strength), ]
   head(top_interactions, 10)
 }
