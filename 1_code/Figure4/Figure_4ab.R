...
gut_oral_results_summary<-merge(gut_oral_results_summary,metabolite_annotation[,c("variable_id","HMDB.Name","HMDB.Source.Microbial","HMDB.Class")],by.x="metabolite",by.y="variable_id")

gut_oral_results_summary<-subset(gut_oral_results_summary,gut_R2>0&oral_R2>0)
...
gut_oral_results_summary_co_influence<-subset(gut_oral_results_summary_co_influence,!(HMDB.Name=="NA"))
...
calculate_feature_importance_proportions <- function(results) {
  ...
  importance_wide <- reshape(importance_summary, 
                             idvar = "metabolite",
...
# 准备用于可视化的数据
viz_data <- melt(viz_data[,c("HMDB.Name", grep("proportion_", names(feature_importance_proportions), value = TRUE))],
                 id.vars = "HMDB.Name",
...
b<-ggplot(viz_data, aes(x = HMDB.Name, y = proportion, fill = feature_type)) +
  ...
  labs(title = "",
       x = "Metabolite",
       y = "Proportion (%)",
       fill = "Feature") +scale_fill_manual(values = c("gut" = "#edd064", 
                                                       "oral" = "#a1d5b9",
                                                       "interaction" = "grey50"))+
  theme(
